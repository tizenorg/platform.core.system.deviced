...
#define USB_BASE        "/etc/deviced/usb-base.conf"
...
static int load_base_config(struct parse_result *result, void *user_data)
{
	...
	if (strncmp(result->section, SECTION_BASE, strlen(result->section)))
		return -EINVAL;

	len = strlen(result->name);

	if (!strncmp(result->name, KEY_ROOTPATH, len)) {
		snprintf(config_rootpath, sizeof(config_rootpath),
				"%s", result->value);
		_I("USB config rootpath(%s)", config_rootpath);
	}

	else if (!strncmp(result->name, KEY_LOAD, len)) {
		snprintf(config_load, sizeof(config_load),
				"%s", result->value);
		_I("USB config load(%s)", config_load);
	}

	else if (!strncmp(result->name, KEY_DEFAULT, len)) {

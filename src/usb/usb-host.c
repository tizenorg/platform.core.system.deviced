...
struct usb_host_device {
	char devnode[PATH_MAX]; /* unique info. */
	int class;
	int subclass;
	int protocol;
	int vendorid;
...
static void uevent_usb_handler(struct udev_device *dev)
{
	...
	/**
	 * Usb device must have at least one interface.
	...
	if (!strncmp(action, UDEV_ADD, sizeof(UDEV_ADD))) {
		...
		/* devnode is an unique information among usb host devices */
		devnode = udev_device_get_devnode(parent);
		if (!devnode) {
			_E("fail to get devnode from udev_device");
			free(usb_host);
			return;
		}

		/* save the devnode */
		strncpy(usb_host->devnode, devnode, sizeof(usb_host->devnode));
		...
		DD_LIST_FOREACH_SAFE(usb_host_list, n, next, usb_host) {
			if (!strncmp(usb_host->devnode,
						devnode, sizeof(usb_host->devnode)))
				break;

			if (!usb_host) {
				_E("fail to find the matched usb host device");
				return;
			}

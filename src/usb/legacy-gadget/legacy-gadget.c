...
#define LEGACY_GADGET_SETTING     "/etc/deviced/legacy-gadget-setting.conf"
#define LEGACY_GADGET_OPERATION   "/etc/deviced/legacy-gadget-operation.conf"
...
static int execute_operation(char *type, char *name)
{
	...
	ret = config_parse(LEGACY_GADGET_OPERATION,
...
static int update_configuration(char *name)
{
	...
	return config_parse(LEGACY_GADGET_SETTING,
...
{
	int ret = 0;
	bool base_config_initialized = false;

	if (!base_config_initialized) {
		ret = config_parse(LEGACY_GADGET_SETTING,
...
static int legacy_gadget_init(char *name)
{
	int ret;
...
static int legacy_gadget_enable(char *name)
{
	int ret;
...
static int legacy_gadget_disable(char *name)
{
	return execute_operation(name, KEY_STOP);
}

static const struct usb_config_plugin_ops legacy_gadget_plugin = {
	.init      = legacy_gadget_init,
	.enable    = legacy_gadget_enable,
	.disable   = legacy_gadget_disable,
};

static bool legacy_gadget_is_valid(void)
{
	struct stat st;
	bool is_valid = false;
	int ret;
...
static const struct usb_config_plugin_ops *legacy_gadget_load(void)
{
	return &legacy_gadget_plugin;
}

static const struct usb_config_ops usb_config_legacy_gadget_ops = {
	.is_valid  = legacy_gadget_is_valid,
	.load      = legacy_gadget_load,
};

USB_CONFIG_OPS_REGISTER(&usb_config_legacy_gadget_ops)

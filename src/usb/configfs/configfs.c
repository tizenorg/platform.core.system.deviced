...
static int load_operation_config(struct parse_result *result, void *user_data)
{
	...
	/* TODO check if strncmp() is really needed */
	if (strncmp(result->section, data->type, strlen(result->section)) ||
	    strncmp(result->name, data->name, strlen(result->name)))
		return 0;
...
static int load_base_config(struct parse_result *result, void *user_data)
{
	...
	if (!strncmp(result->name, KEY_CFSROOT, sizeof(KEY_CFSROOT))) {
		snprintf(cfs_root, sizeof(cfs_root), "%s", result->value);
		_I("USB config cfsroot(%s)", cfs_root);
	} else if (!strncmp(result->name, KEY_UDC, sizeof(KEY_UDC))) {

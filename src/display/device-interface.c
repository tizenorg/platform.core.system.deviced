...
{
	if (!display_dev || !display_dev->set_power_state) {
		_E("there is no display device");
		return -ENOENT;
	}

	return display_dev->set_power_state(on);
}
...
{
	enum display_power power;
	int ret;

	if (!display_dev || !display_dev->get_power_state) {
		_E("there is no display device");
		return -ENOENT;
	}

	ret = display_dev->get_power_state(&power);
	if (ret < 0)
		return ret;
...
static int backlight_on(enum device_flags flags)
{
	...
	for (i = 0; i < PM_LCD_RETRY_CNT; i++) {
		ret = pmsys->bl_onoff(pmsys, DISPLAY_POWER_ON);
		if (get_lcd_power() == DISPLAY_POWER_ON) {
#ifdef ENABLE_PM_LOG
...
#endif
			usleep(30000);
		ret = pmsys->bl_onoff(pmsys, DISPLAY_POWER_OFF);
		if (get_lcd_power() == DISPLAY_POWER_OFF) {
#ifdef ENABLE_PM_LOG
...
static int backlight_standby(int force)
{
	...
	if ((get_lcd_power() == DISPLAY_POWER_ON) || force) {
		_I("LCD standby");
		ret = pmsys->bl_onoff(pmsys, DISPLAY_POWER_STANDBY);
	}

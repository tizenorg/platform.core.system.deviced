...
static void print_buffer(const unsigned char *vibe_buffer)
{
	...
	memcpy(&fb.data, (unsigned char *)vibe_buffer+sizeof(struct ff_info_header), sizeof(struct ff_info_data) * fb.header.ff_info_data_count);
	_D("\nMagic %x\niteration %d\ncount %d\n", fb.header.magic, fb.header.iteration, fb.header.ff_info_data_count);

	for (i = 0; i < fb.header.ff_info_data_count; i++)
		_D("type %d\nmagn 0x%x\nlen %d\n", fb.data[i].type, fb.data[i].magnitude, fb.data[i].length);
}

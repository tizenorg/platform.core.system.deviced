...
static Eina_Bool uevent_control_cb(void *data, Ecore_Fd_Handler *fd_handler)
{
	...
	if ((dev = udev_monitor_receive_device(info->mon)) == NULL)
		return ECORE_CALLBACK_RENEW;
		...
		struct uevent_info *info)
{
	...
	if (udev_monitor_set_receive_buffer_size(info->mon, UDEV_MONITOR_SIZE) != 0) {
		_E("fail to set receive buffer size");
		goto stop;
	}

	DD_LIST_FOREACH(info->event_list, elem, l) {
		ret = udev_monitor_filter_add_match_subsystem_devtype(info->mon,
				l->subsystem, NULL);
		if (ret < 0) {
	...
	info->fd = udev_monitor_get_fd(info->mon);
	if (info->fd == -1) {
		_E("error udev_monitor_get_fd");
		goto stop;
	}

	info->fdh = ecore_main_fd_handler_add(info->fd, ECORE_FD_READ,

...
static void invoke_name_owner_changed(void)
{
	...
	DD_LIST_FOREACH(edbus_watch_list, n, watch) {
		if (!watch->deleted &&
			...
			DD_LIST_FOREACH(watch->func_list, elem, func)
				func(watch->sender, watch->type);
		}
	}
}
		...
		void (*func)(const char *sender, enum watch_type type))
{
	...
	DD_LIST_APPEND(watch->func_list, func);

	_I("%s(%d) is watched by dbus!", watch->sender, watch->type);
